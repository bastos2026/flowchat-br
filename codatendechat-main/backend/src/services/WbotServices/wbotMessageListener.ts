...
const verifyQueue = async (
  ...
  // FLOWCHAT BR: SISTEMA COMPLETAMENTE SEM FILAS - ATENDIMENTO DIRETO
  const sendGreetingMessage = await Setting.findOne({
    where: {
  ...
  if (
    ...
    sendGreetingMessage?.value === "enabled"
  ) {
  ...
  // Verificar integrações diretas no WhatsApp (sem fila)
  const whatsapp = await Whatsapp.findByPk(wbot.id!);
  
  // Iniciar integração dialogflow/n8n se configurada no WhatsApp
  if (
    !msg.key.fromMe &&
    !ticket.isGroup &&
  ...
  // Iniciar integração openai se configurada no WhatsApp
  if (!msg.key.fromMe && !ticket.isGroup && !isNil(whatsapp?.promptId)) {
  ...
  // Enviar mensagem de boas-vindas do WhatsApp se configurada
  ...
  // Direcionar para atendimento direto (sem fila)
  await UpdateTicketService({
    ticketData: { 
      queueId: null, 
      chatbot: false, 
...
// FLOWCHAT BR: SISTEMA SEM FILAS - CÓDIGO REMOVIDO
// O sistema agora funciona com atendimento direto, sem menus de seleção
};
...
// FLOWCHAT BR: FUNÇÃO REMOVIDA - SISTEMA SEM FILAS
// A função flowBuilderQueue foi removida pois não há mais sistema de filas
// O atendimento agora é direto, sem menus de seleção
...
const handleMessage = async (
  ...
  try {
    ...
    // FLOWCHAT BR: SISTEMA SEM FILAS - REMOVIDO
    // Não há mais menu inicial para voltar
    ...
    // FLOWCHAT BR: SISTEMA SEM FILAS - REMOVIDO
    // As integrações agora são configuradas diretamente no WhatsApp

    // FLOWCHAT BR: SISTEMA SEM FILAS - REMOVIDO
    // As integrações agora são configuradas diretamente no WhatsApp

    // FLOWCHAT BR: SISTEMA SEM FILAS - ATENDIMENTO DIRETO
    if (
      !ticket.isGroup &&
      !msg.key.fromMe &&
      !ticket.userId &&
    ...
    // FLOWCHAT BR: SISTEMA SEM FILAS - REMOVIDO
    // Não há mais sistema de filas, chatbot ou menus
